...
# lookup the public IP address of the launched instance from the EC2 Dashboard
...
* Amendment of cloudformation script ( json or yaml style ) add AMI golden image to use
...
* performance expectations are that with a single node and 1 cpu per task and 16 tasks per node we can run 20000 sampling_frame in 10 hours
