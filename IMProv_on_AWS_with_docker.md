...
* IAM role for s3 upload of modeling results. 
  * NOTE: add scripted copy of modeling output data to s3.
...
When we are inside the docker container and have changed folder directory to reach the imp_model folder [eg: docker exec -it a848d063379e bash ]
We can call the aws_run_onenode.sh script by providing a runnum as the first parameter.
cmd: ./aws_run_onenode.sh 231
...
We call it and provide a parameter for the runnum: ./jcl_docker.sh 241
